var typed = new Typed('.typed', {
    strings: ["Բարի <span>գալուստ</span> FootballTime"],
    typeSpeed: 100
  });
  ...
  function countdown() {
      ...
      const totalSeconds = (worldCupDate - currentDate) / 1000; // miliseconds
...
//       // let goalRatio = (value.goals / value.games).toFixed(2);
//       gsTable += '<tr>';
//       gsTable += `<td><img class="table_img" src="img/${value.img}" alt=""></td>`;
//       gsTable += `<td>${value.name}</td>`;
//       gsTable += `<td>${value.team}</td>`;
//       gsTable += `<td>${value.games}</td>`;
//       gsTable += `<td>${value.goals}</td>`;
//       gsTable += `<td>${value.assists}</td>`;
//       gsTable += '</tr>';
//       if (value.id >= 5) {
//         return false;
//       }
//     });
//     $('.gscorer_table').append(gsTable);
// });
...
function init () {
    navigator.geolocation.getCurrentPosition(updateLocation, handleError);
            // ստանում ենք կոորդինատները
            function updateLocation(position) {
                const latitude = position.coords.latitude;
                const longitude = position.coords.longitude;
                
                if ((localStorage.getItem('ourLatitude') == undefined) || (localStorage.getItem('ourLongitude') == undefined)) {
                    localStorage.setItem('ourLatitude', latitude);
                    localStorage.setItem('ourLongitude', longitude);
                }
            ...
            let ourLatitude = localStorage.getItem('ourLatitude');
            let ourLongitude = localStorage.getItem('ourLongitude');
            console.log(ourLatitude);
            console.log(ourLongitude);
